...
uint8_t raw_scan_rsp_data[100] = {
        0x0f, 0x09, 0x45, 0x53, 0x50, 0x5f, 0x47, 0x41, 0x54, 0x54, 0x53, 0x5f, 0x44, 0x45, 0x4d, 0x4d
};
#else
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_REG_EVT:
        ...
        adv_config_done |= adv_config_flag;
        esp_err_t raw_scan_ret = esp_ble_gap_config_scan_rsp_data_raw(raw_scan_rsp_data, raw_scan_rsp_data[0]+1);
        if (raw_scan_ret){
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        if (!param->write.is_prep){
            ...
            printf("recv = 0x%02x", *param->write.value );

            if(*param->write.value == 0x11)
            {
                Led_CTL_R_On();
            }
            else if(*param->write.value == 0x22)
            {
                Led_CTL_B_On();
            }
            else if(*param->write.value == 0x00)
            {
                Led_CTL_Off();
            }
